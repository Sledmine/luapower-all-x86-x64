...
local https_socket = require("socket.https")
...
function _requests.make_request(request)
    ...
    local response = {}
    local ok
    local socket = string.find(full_request.url, "^https:") and not request.proxy and
                       https_socket or http_socket
...
function _requests.check_redirect(allow_redirects)
    if allow_redirects and type(allow_redirects) ~= "boolean" then
        error("allow_redirects expects a boolean value. received type = " ..
...
function _requests.digest_create_header_string(auth)
    local authorization = "Digest username=\"" .. auth.user .. "\", realm=\"" ..
                              auth.realm .. "\", nonce=\"" .. auth.nonce
    authorization = authorization .. "\", uri=\"" .. auth.uri .. "\", qop=" .. auth.qop ..
                        ", nc=" .. auth.nc
    authorization = authorization .. ", cnonce=\"" .. auth.cnonce .. "\", response=\"" ..
                        auth.response .. "\""
...
function _requests.digest_hash_response(auth_table)
    return md5_hash(md5_hash(auth_table.user, auth_table.realm, auth_table.password),
                    auth_table.nonce, auth_table.nc, auth_table.cnonce, auth_table.qop,
